...
CREATE INDEX idx_pools_name ON desktop_pools(name) WHERE deleted_at IS NULL;
...
CREATE INDEX idx_pools_search ON desktop_pools USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '')));
...
CREATE INDEX idx_instances_pool_status ON desktop_instances(pool_id, status);  -- Composite for queries
CREATE INDEX idx_instances_assigned_user ON desktop_instances(assigned_user_id) WHERE assigned_user_id IS NOT NULL;
CREATE INDEX idx_instances_openstack_id ON desktop_instances(openstack_vm_id) WHERE openstack_vm_id IS NOT NULL;
CREATE INDEX idx_instances_floating_ip ON desktop_instances(floating_ip) WHERE floating_ip IS NOT NULL;
...
CREATE INDEX idx_jobs_instance_id ON provisioning_jobs(instance_id) WHERE instance_id IS NOT NULL;
CREATE INDEX idx_jobs_status ON provisioning_jobs(status);
CREATE INDEX idx_jobs_status_queued ON provisioning_jobs(status, queued_at) WHERE status = 'queued';  -- For job processing
...
CREATE TABLE user_assignments (
    ...
    -- Assignment Type
    assignment_type VARCHAR(50) NOT NULL CHECK (assignment_type IN ('persistent', 'temporary')),
...
CREATE INDEX idx_assignments_active ON user_assignments(user_id, released_at) WHERE released_at IS NULL;  -- Active sessions
...
CREATE TABLE scaling_policies (
    ...
    scale_up_threshold_percent INTEGER DEFAULT 20 CHECK (scale_up_threshold_percent BETWEEN 0 AND 100),
    scale_up_increment INTEGER DEFAULT 2 CHECK (scale_up_increment > 0),
    
    -- Scale Down Configuration
    scale_down_threshold_percent INTEGER DEFAULT 50 CHECK (scale_down_threshold_percent BETWEEN 0 AND 100),
...
CREATE INDEX idx_policies_enabled ON scaling_policies(enabled) WHERE enabled = TRUE;
...
CREATE INDEX idx_health_unhealthy ON vm_health_checks(is_healthy, checked_at) WHERE is_healthy = FALSE;
...
CREATE TABLE instance_snapshots (
    ...
    snapshot_type VARCHAR(50) NOT NULL CHECK (snapshot_type IN ('manual', 'auto', 'pre_update')),
    size_bytes BIGINT CHECK (size_bytes >= 0),
    status VARCHAR(50) DEFAULT 'creating' CHECK (status IN ('creating', 'available', 'error', 'deleting')),
...
CREATE INDEX idx_snapshots_openstack_id ON instance_snapshots(openstack_snapshot_id) WHERE openstack_snapshot_id IS NOT NULL;
...
SELECT 
    ...
    p.current_count,
    p.status,
    COUNT(CASE WHEN di.status = 'ready' THEN 1 END) AS available_vms,
    COUNT(CASE WHEN di.status = 'in_use' THEN 1 END) AS in_use_vms,
    COUNT(CASE WHEN di.status = 'error' THEN 1 END) AS error_vms,
    COUNT(CASE WHEN di.status = 'stopped' THEN 1 END) AS stopped_vms,
    CASE 
        WHEN p.current_count > 0 THEN 
            ROUND((COUNT(CASE WHEN di.status = 'in_use' THEN 1 END)::NUMERIC / p.current_count) * 100, 2)
        ELSE 0 
    END AS utilization_percent,
    COUNT(DISTINCT CASE WHEN ua.released_at IS NULL THEN ua.user_id END) AS active_users
FROM desktop_pools p
LEFT JOIN desktop_instances di ON p.pool_id = di.pool_id
LEFT JOIN user_assignments ua ON di.instance_id = ua.instance_id AND ua.released_at IS NULL
WHERE p.deleted_at IS NULL
GROUP BY p.pool_id, p.name, p.desktop_type, p.min_vms, p.max_vms, p.current_count, p.status;
...
SELECT 
    ...
    ua.assigned_at,
    EXTRACT(EPOCH FROM (NOW() - ua.assigned_at)) AS session_duration_seconds,
    p.pool_id,
    p.name AS pool_name
...
SELECT 
    ...
    COUNT(CASE WHEN vhc.is_healthy = FALSE THEN 1 END) AS unhealthy_instances,
...
LEFT JOIN LATERAL (
    ...
    WHERE instance_id = di.instance_id
    ORDER BY checked_at DESC
    LIMIT 1
) vhc ON TRUE
WHERE p.deleted_at IS NULL
GROUP BY p.pool_id, p.name, p.status;
...
('max_concurrent_users', '50', 'Maximum concurrent users allowed'),
('default_session_timeout', '240', 'Default session timeout in minutes'),
('min_password_length', '8', 'Minimum password length requirement'),
('enable_auto_scaling', 'true', 'Global auto-scaling toggle');
...
VALUES ('schema_version', '1.0.0', 'Database schema version')
